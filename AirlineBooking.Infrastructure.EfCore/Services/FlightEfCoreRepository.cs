...
public class FlightEfCoreRepository(AirlineBookingDbContext context) : IFlightRepository
{
    ...
    {
        var result = await _flights.AddAsync(entity);
    ...
    {
        var entity = await _flights.FirstOrDefaultAsync(e => e.Id == key);
        if (entity == null)
            return false;
